...
public class MainFrame extends JFrame {
	...
	private String helpURL = "https://github.com/oscarkarnalim/cstrange";
	...
	private JCheckBox simSurfaceField, simSyntacticField, simSemanticField;
	...
	public MainFrame() {
		...
		simSemanticField = new JCheckBox("Pseudo-semantic");
		simSemanticField.setSelected(true);
		simSemanticField.setToolTipText("JPlag does not work on overly short submissions");
		simSemanticField.setFont(new Font("Times New Roman", Font.PLAIN, 14));
		simSemanticField.setBounds(250, 470, 130, 30);
		contentPane.add(simSemanticField);

		JLabel lblReportedSimilarities = new JLabel("Reported similarities :");
		...
		lblReportedSimilarities.setBounds(20, 470, 142, 30);
		contentPane.add(lblReportedSimilarities);

		simSyntacticField = new JCheckBox("Syntactic");
		simSyntacticField.setSelected(true);
		simSyntacticField.setFont(new Font("Times New Roman", Font.PLAIN, 14));
		simSyntacticField.setBounds(382, 470, 86, 30);
		contentPane.add(simSyntacticField);

		simSurfaceField = new JCheckBox("Surface");
		simSurfaceField.setSelected(true);
		simSurfaceField.setFont(new Font("Times New Roman", Font.PLAIN, 14));
		simSurfaceField.setBounds(470, 470, 86, 30);
		contentPane.add(simSurfaceField);
	...
	private void submit() {
		...
		if (simSurfaceField.isSelected() == false && simSyntacticField.isSelected() == false
				&& simSemanticField.isSelected() == false)
			// if no similarity types selected for reported similarities
			errorMessage += "Please select at least one similarity to be reported\n";

		if (progLangField.getSelectedIndex() >= 2) {
			// if the selected language is other than Java and Python
			if (simSyntacticField.isSelected())
				// if syntactic is selected
				errorMessage += "The tool does not support syntactic similarity for the programming language\n";

			if (simSurfaceField.isSelected())
				// if surface is selected
				errorMessage += "The tool does not report surface similarity for the programming language\n";

			if (commonCodeField.getSelectedIndex() == 1)
				// if user wants to disallow common code
				errorMessage += "The tool does not support common code removal for the programming language\n";
		}
		...
		if (errorMessage.length() > 0) {
			...
			String resultPath = process(assignmentPath, submissionTypeField.getSelectedItem().toString(),
					...
					simSemanticField.isSelected(), simSyntacticField.isSelected(), simSurfaceField.isSelected());
	...
	public String process(String assignmentPath, String submissionType, String progLang, String humanLang,
			...
			boolean isSemanticSimReported, boolean isSyntacticSimReported, boolean isSurfaceSimReported) {
...
//		System.setErr(dummyStream);
		...
		if (isSemanticSimReported) {
			// if semantic comparison is checked, use it as the main measurement
			String errorMessage = PseudoSemanticComparer.doSemanticComparison(assignmentPath, progLang, humanLang,
					simThreshold, maxPairs, minMatchingLength, templateDirPath, isSyntacticSimReported,
					isSurfaceSimReported, assignmentParentDirPath, assignmentName, isMultipleFiles, isCommonCodeAllowed,
					additionalKeywordsPath, filesToBeDeleted);

			if (errorMessage != null) {
				System.out.println(errorMessage);
				if (errorMessage.startsWith("Not enough valid submissions!"))
					errorMessage = "JPlag does not identify enough compilable submissions with reasonable length\nTry to lower the minimum matching length or avoid using pseudo-semantic similarity";

				JOptionPane.showMessageDialog(this, errorMessage, "Error(s)", JOptionPane.ERROR_MESSAGE);

				// return null if the generation fails
				return null;
			}

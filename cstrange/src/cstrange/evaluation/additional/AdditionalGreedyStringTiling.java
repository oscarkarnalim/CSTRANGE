...
public class AdditionalGreedyStringTiling {

	// data token
	private static char[] T;
	private static char[] P;
	// menyimpan hasil sementara
	private static ArrayList<Queue<AdditionalGSTMatchTuple>> matchList;
	 ...
	 * similarity
	 */
	public static double calcAverageSimilarity(ArrayList<AdditionalGSTMatchTuple> tiles) {
	...
	public static double calcAverageIDFSimilarity(
			ArrayList<AdditionalGSTMatchTuple> tiles, HashMap<String, Double> index1,
		...
		for (int i = 0; i < tiles.size(); i++) {
			AdditionalGSTMatchTuple tile = tiles.get(i);
	...
	public static double calcAverageSimilarityWithPenalty(
			ArrayList<AdditionalGSTMatchTuple> tiles) {
	 ...
	 * similarity
	 */
	public static double calcMaximumSimilarity(ArrayList<AdditionalGSTMatchTuple> tiles) {
	 ...
	 */
	public static int coverage(ArrayList<AdditionalGSTMatchTuple> tiles) {
		int accu = 0;
		for (int i = 0; i < tiles.size(); i++) {
			AdditionalGSTMatchTuple tile = tiles.get(i);
			accu += tile.length;
	...
	public static double coverageIDF(ArrayList<AdditionalGSTMatchTuple> tiles,
	 ...
	 * matching
	 */
	public static ArrayList<AdditionalGSTMatchTuple> getMatchedTiles(char[] s1,
			char[] s2, int minimalMatchingLength) {
		P = s1;
		T = s2;
		// renew all data structure needed
		ArrayList<AdditionalGSTMatchTuple> tiles = new ArrayList<AdditionalGSTMatchTuple>();
		matchList = new ArrayList<Queue<AdditionalGSTMatchTuple>>();
		// renew the marker
		...
		// optimised so that it runs only once
	...
	public static ArrayList<AdditionalGSTMatchTuple> getMatchedTilesWithPredefinedMarkedArrays(char[] newP, boolean[] newMarkedP,
			char[] newT, boolean[] newMarkedT, int minimalMatchingLength) {
		// renew all data structure needed
		ArrayList<AdditionalGSTMatchTuple> tiles = new ArrayList<AdditionalGSTMatchTuple>();
		matchList = new ArrayList<Queue<AdditionalGSTMatchTuple>>();
	...
	public static ArrayList<AdditionalGSTMatchTuple> getMatchedTilesWithRemainingUnmatchedRegion(
			char[] s1, char[] s2, int minimalMatchingLength) {
		/*
	...
	private static void scanpattern(int s) {
		Queue<AdditionalGSTMatchTuple> queue = new LinkedList<AdditionalGSTMatchTuple>();
		AdditionalGSTHashTable hashtable = new AdditionalGSTHashTable();
		/*
					...
					for (Integer tt : values) {
						...
						if (newsb.toString().equals(substring)) {
							...
							while (p + k < P.length && tt + k < T.length
								...
								k = k + 1;
							// masukkan ke queue
							AdditionalGSTMatchTuple mv = new AdditionalGSTMatchTuple(p, tt, k);
							queue.add(mv);
						}
					}
				}
	 ...
	 */
	private static void markStrings(int s, ArrayList<AdditionalGSTMatchTuple> tiles) {
		for (Queue<AdditionalGSTMatchTuple> queue : matchList) {
			while (!queue.isEmpty()) {
				AdditionalGSTMatchTuple match = queue.poll();
	 ...
	 */
	private static boolean isOccluded(AdditionalGSTMatchTuple match,
			ArrayList<AdditionalGSTMatchTuple> tiles) {
		if (tiles.equals(null) || tiles == null || tiles.size() == 0)
			return false;
		for (AdditionalGSTMatchTuple matches : tiles) {

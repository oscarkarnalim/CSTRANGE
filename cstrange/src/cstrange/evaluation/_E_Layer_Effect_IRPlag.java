...
public class _E_Layer_Effect_IRPlag {
	public static void main(String[] args) {
		String rootdirpath = "C:\\Users\\oscar\\Desktop\\IR-Plag";
		String assessmentRooPath = "C:\\Users\\oscar\\Desktop\\IR-Plag\\[formatted for JPlag] IR-Plag-Dataset";
		...
		// get jplag dirs
		File[] dirs = new File(rootdirpath).listFiles();
		for (int i = 0; i < dirs.length; i++) {
			if (dirs[i].getName().contains("[out]")) {
				String[] split = dirs[i].getName().split(" ");
				int threshold = Integer.parseInt(split[1]);
				extract(dirs[i].getAbsolutePath() + File.separator + "index.html",
						assessmentRooPath + File.separator + split[2] + File.separator + split[3], split[2], split[3],
						threshold, additionalKeywords);
			}
		}
		System.out.println(System.currentTimeMillis() - before);
	}

	private static void extract(String jplagIndexPath, String assessmentPath, String taskName, String level,
			int threshold, ArrayList<ArrayList<String>> additionalKeywords) {
...
//			double recall = copiedAndSuggested * 1.0 / totalCopied;
